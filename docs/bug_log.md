...
| Atributo | Detalle |
| :--- | :--- |
| **ID / Título** | 78. Función solicitada: renombrar pestañas desde el menú contextual |
| **Estado** | [Pendiente de validación del usuario] |
| **Descripción / Síntomas** | Los títulos solo podían modificarse desde el editor modal completo o desde el código fuente. Se solicitó una acción específica de renombrado en el menú contextual de cada pestaña. |
| **Causa Raíz Identificada** | N/A |
| **Solución / Implementación** | El menú incorpora «Renombrar pestaña» entre las acciones de añadir y eliminar, con icono de lápiz y desactivación automática si el clic no corresponde a una pestaña válida. La acción abre un modal compacto con el título fuente actual seleccionado, permite confirmar con `Enter` y cancelar con `Escape`, mantiene el modal abierto ante errores y rechaza nombres vacíos.<br>`renameTabAt()` reemplaza exclusivamente el rango del título posterior al identificador configurado. No reconstruye la sección ni modifica contenido, cercas, configuración, saltos de línea o bloques descendientes. La operación exige concordancia fuente-DOM, valida nuevamente la cantidad de secciones y reutiliza la propagación atómica hijo-padre-raíz.<br>**Continuidad de caché:** Como las identidades de descendientes incluyen el título y su ocurrencia, renombrar —especialmente hacia o desde nombres duplicados— puede cambiar varias rutas. Esas rutas se proyectan y migran antes de escribir para que un render síncrono encuentre los índices activos correctos. La migración se revierte ante rechazo o excepción y elimina claves antiguas solo después de una escritura confirmada. |
| **Pruebas / Verificación** | El modal captura una instantánea inmutable que referencia la fuente exacta sin duplicarla. Si cualquier parte del bloque cambia antes de confirmar, el renombrado se cancela en vez de reutilizar un índice potencialmente obsoleto. Las postcondiciones comprueban que el título nuevo sea el único intervalo diferente y que el analizador conserve prefijo y cantidad de secciones. Se rechazan saltos Unicode y caracteres de control, bloques DOM descargados y operaciones concurrentes con el editor modal completo del mismo árbol.<br>82 aserciones adversariales y estructurales, seguidas por una pasada final de 23 aserciones de regresión integrada, sobre cambio aislado, snapshot obsoleto, DOM descargado, sincronía entre navegación/contenidos/fuente, índices fraccionarios o inválidos, conflicto con editor modal, control de caracteres, postcondiciones, títulos duplicados, migración y rollback de caché, excepción simulada, propagación anidada hasta la raíz, escritura única, foco de hijo/padre y comportamiento del modal; además de `node --check main.js` y `git diff --check`. |
