...
export class TabsRenderer extends MarkdownRenderChild {
  constructor(t, e, i, n, r, isVertical = false) {
    ...
    if (this.currentIndex >= this.tabsNav.navItems.length) {
      this.currentIndex = Math.max(0, this.tabsNav.navItems.length - 1);
    ...
    switch (
      ...
      this.tabsType === "outertabs" &&
        ...
        this.registerDomEvent(
          ...
          (t) => {
            (t.preventDefault(),
              !(!this.activeView || this.isPreviewMode()) &&
      ...
      default:
        ((this.plugin.settings.actionButtonType = "action-none"),
          ...
          new ws.Notice($("notice.invalidActionButtonType")));
    }
    (this.activeView &&
      !this.isPreviewMode() &&
